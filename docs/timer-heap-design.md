...
At the top of every loop iteration the receiver thread consults the heap: ```java long timeoutNs = 
timerWheel.earliestDeadline() - System.nanoTime(); if (timeoutNs <= 0) { handleTimeout(); continue; }
socket.setSoTimeout((int) TimeUnit.NANOSECONDS.toMillis(timeoutNs)); socket.receive(packet);
