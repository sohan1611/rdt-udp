...
## 7. Schedule (revised 14 September 2026)

Mid-semester exams took 1–14 September for every student, so no work happened in the original weeks 1
and 2. The team recovers it with a **two-week sprint, Mon 14 – Sun 27 September**, covering original
weeks 1–5. The week after is a buffer for the long experiment runs, and from **6 October** the original
dates resume unchanged. Each period ends at a **gate** — a binary, testable condition. The day-by-day
plan for each member is in `docs/member-playbook.html`.

| Period | Dates | Target | Gate |
|---|---|---|---|
| **Sprint A** | Sep 14–20 | `Packet` rewrite, metadata handshake, Stop-and-Wait + CLIs; emulator rewrite, WSL2 calibration, `RunStats`; `TimerWheel`, `RttEstimator`; SR window-bound note, standalone receiver buffer | Tue 15: all four committed · Thu 17: `Packet` and emulator merged · Sun 20: 8 MiB Stop-and-Wait transfer matches at p=0 |
| **Sprint B** | Sep 21–27 | Go-Back-N; Selective Repeat; adaptive and fixed RTO; FIN teardown and shared integrity suite; `run_matrix.py`, `plots.py`; experiments 1–3 first pass | Tue 22: SR fallback decision · **Thu 24: Core complete** — all 3 protocols pass one shared integrity suite · Sun 27: figures 1–3 by script, ≥3 seeds per cell |
| **Buffer** | Sep 28–Oct 5 | Experiments 1–3 at full 10 seeds; report methodology; anything that slipped | Figures 1–3 final, from committed CSVs |
...
| **W8** | Oct 20–26 | Code freeze Oct 22; final report; viva drills | `make all` reproduces every figure from a clean clone |
| **Submit** | Oct 27–31 | Submission | — |
...
**Risk 1 — Selective Repeat overruns the sprint.** The receiver buffer plus per-packet timers is the single
hardest piece.
...
fallback on **Tuesday 22 September** if SR does not transfer a file correctly at 10% loss — two days before Core complete, not in October.
